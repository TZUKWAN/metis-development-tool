...
2. **OpenAI Codex** (`packages/mdt-codex`): MDT's _internal_ coding agent.
...
3. **Pi Agent Core** (`packages/mdt-pi-runtime`,
   `@mariozechner/pi-agent-core`): the _runtime_ of the agent apps users
...
- _Use Codex as the user-facing runtime_: rejected — vendor lock-in, wrong
  abstraction, Codex is a coding agent not an app runtime.
- _Use the Pi coding CLI as MDT's builder_: rejected — Pi's CLI agent is a
  general coding agent; MDT's builder must speak the Blueprint contract and
  the task explicitly assigns that role to Codex.
- _Reimplement the agent loop in MDT_: rejected — duplicated maintenance,
