...
function sandboxResolve(ctx: CapabilityContext, userPath: unknown, label: string): string {
  ...
  if (ctx.sandboxRoots.length === 0) {
    throw new PathEscapeError(`${label}: no sandbox roots configured in this project — cannot resolve "${userPath}"`)
  }
...
export const fileReadCapability: Capability = {
  manifest: {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        maxBytes: { type: 'number', description: 'refuse files larger than this (default 1000000)' },
      },
      required: ['path'],
    ...
    permissions: [{ scope: 'filesystem', detail: 'read files inside the sandbox', required: true, defaultGranted: false }],
    secrets: [],
    ui: {
      icon: '📄',
      accent: '#5b8def',
  ...
  async execute(input: Record<string, unknown>, ctx: CapabilityContext): Promise<Record<string, unknown>> {
    ...
    if (stat.isDirectory()) throw new Error(`file_read: "${input.path}" is a directory — file_list can enumerate it`)
...
export const fileWriteCapability: Capability = {
  manifest: {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        mkdirs: { type: 'boolean', description: 'create missing parent directories (default true)' },
      },
    ...
    permissions: [{ scope: 'filesystem', detail: 'write files inside the sandbox', required: true, defaultGranted: false }],
    secrets: [],
    ui: {
      icon: '✏️',
      accent: '#5b8def',
  ...
  async execute(input: Record<string, unknown>, ctx: CapabilityContext): Promise<Record<string, unknown>> {
    ...
    if (input.content === undefined || input.content === null) throw new Error('file_write: "content" is required')
    ...
    if (existed && !overwrite) {
      throw new Error(`file_write: "${input.path}" already exists — pass overwrite: true to replace it`)
    }
    if (existed) {
      const stat = await fsp.stat(resolved)
      if (stat.isDirectory()) throw new Error(`file_write: "${input.path}" is a directory and cannot be written as a file`)
    }
    const buffer = encoding === 'base64' ? Buffer.from(String(input.content), 'base64') : Buffer.from(String(input.content), 'utf8')
    const parent = path.dirname(resolved)
    if (!existsSync(parent)) {
      if (!mkdirs) throw new Error(`file_write: parent directory "${parent}" does not exist and mkdirs=false`)
...
export const fileListCapability: Capability = {
  manifest: {
    ...
    permissions: [{ scope: 'filesystem', detail: 'list files inside the sandbox', required: true, defaultGranted: false }],
    secrets: [],
    ui: {
      icon: '🗂️',
      accent: '#5b8def',
  ...
  async execute(input: Record<string, unknown>, ctx: CapabilityContext): Promise<Record<string, unknown>> {
    ...
    const userPath = input.path === undefined || input.path === null || input.path === '' ? '.' : input.path
    ...
    const walk = async (dir: string, level: number): Promise<void> => {
      ...
      for (const dirent of dirents) {
        ...
        entries.push({ name: dirent.name, path: real, type: isDir ? 'dir' : 'file', size: isDir ? 0 : stat.size })
        if (isDir && level + 1 < depth) await walk(real, level + 1)
      }
    }
    await walk(resolved, 0)
    ctx.log(`file_list ${resolved} depth=${depth} entries=${entries.length}${truncated ? ' (truncated)' : ''}`)
