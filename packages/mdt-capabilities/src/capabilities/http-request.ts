...
export const httpRequestCapability: Capability = {
  manifest: {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        headers: { type: 'object', description: 'record of header name → value; values may reference ${secret:NAME}' },
    ...
    permissions: [{ scope: 'network', detail: 'http(s) requests', required: true, defaultGranted: true }],
    secrets: [],
    ui: {
      icon: '📡',
      accent: '#5b8def',
  ...
  async execute(input: Record<string, unknown>, ctx: CapabilityContext): Promise<Record<string, unknown>> {
    ...
    const rawUrl = requireNonEmptyString(input.url, 'http_request: "url" is required and must be a non-empty string')
    ...
    const requestBody = input.json !== undefined ? JSON.stringify(input.json) : input.body === undefined ? undefined : String(input.body)
    ...
    for (const [key, value] of Object.entries(asStringRecord(input.query, 'http_request: "query"'))) {
    ...
    ctx.log(`http_request ${method} ${redact(target.href)} -> ${result.status} bytes=${result.bytes}`)
    ...
    if (contentType.includes('json')) {
      ...
      try {
        ...
        throw new Error(`http_request: response declared json but the body failed to parse: ${(err as Error).message}`)
      }
...
function normalizeMethod(value: unknown): string {
  const method = value === undefined || value === null || value === '' ? 'get' : String(value).toLowerCase()
  if (!(HTTP_METHODS as readonly string[]).includes(method)) {
    throw new Error(`http_request: method must be one of ${HTTP_METHODS.join('/')} — got "${method}"`)
  }
  return method.toUpperCase()
}

/** Replace `${secret:NAME}` header values from ctx.secrets; missing → clear error. */
function resolveSecretHeaders(headers: Record<string, string>, ctx: CapabilityContext): Record<string, string> {
