...
function parseConfig(value: unknown, server: string): McpConfig {
  if (typeof value !== 'object' || value === null || Array.isArray(value)) {
    throw new Error(
      `mcp: server "${server}" has no capability config — pass input.config with at least { command }`,
    )
  }
  const raw = value as Record<string, unknown>
  if (raw.env !== undefined) {
    throw new Error(
      'mcp: config.env is not supported — the server env comes from the capability context allowlist',
    )
  }
  const transportRaw =
    raw.transport === undefined || raw.transport === '' ? 'stdio' : String(raw.transport)
...
export const mcpCapability: Capability = {
  manifest: {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        config: {
          type: 'object',
          description:
            'server config: { command, args?, transport? (stdio default), url?, timeoutMs? }',
          properties: {
            command: { type: 'string' },
    ...
    outputSchema: {
      ...
      properties: {
        result: {
          description: 'the tools/call result object from the MCP server (includes content)',
        },
      },
      required: ['result'],
      additionalProperties: false,
    ...
    permissions: [
      {
        scope: 'process',
        detail: 'spawn and speak JSON-RPC to a configured MCP server (external integration)',
        required: true,
  ...
  async execute(
    ...
    const server =
      typeof input.server === 'string' && input.server.trim() !== '' ? input.server.trim() : null
    if (!server) throw new Error('mcp: "server" is required and must be a non-empty string')
    const tool =
      typeof input.tool === 'string' && input.tool.trim() !== '' ? input.tool.trim() : null
    ...
    ctx.log(
      `mcp server=${server} tool=${tool} transport=${config.transport} command=${config.command}`,
    )
...
async function callToolOverStdio(opts: {
  ...
  child.on('error', (err) => {
    fail(
      new Error(
        `mcp: failed to start MCP server "${opts.server}" (command "${opts.command}"): ${err.message}`,
      ),
    )
  })
  child.on('exit', (code) => {
  ...
  child.stdout?.on('data', (chunk: Buffer) => {
    ...
    for (;;) {
      ...
      if (message.error) {
        entry.reject(
          new Error(
            `mcp: server "${opts.server}" returned error ${String(message.error.code ?? '')}: ${String(message.error.message ?? 'unknown error')}`,
          ),
        )
      } else {
  ...
  const request = (
    ...
    return new Promise<Record<string, unknown>>((resolve, reject) => {
      ...
      const timer = setTimeout(() => {
        ...
        reject(
          new Error(
            `mcp: ${what} timed out after ${opts.timeoutMs}ms — the server will be terminated`,
          ),
        )
      }, opts.timeoutMs)
      ...
      child.stdin?.write(`${JSON.stringify({ jsonrpc: '2.0', id, method, params })}\n`, (err) => {
        if (err && pending.has(id)) {
          ...
          reject(
            new Error(
              `mcp: failed to write "${method}" to server "${opts.server}": ${err.message}`,
            ),
          )
        }
      })
  ...
  try {
    await request(
      'initialize',
      {
        ...
        capabilities: {},
        clientInfo: { name: 'mdt-capabilities', version: '1.0.0' },
      },
    ...
    const callResult = await request(
      'tools/call',
      { name: opts.tool, arguments: opts.toolArgs },
